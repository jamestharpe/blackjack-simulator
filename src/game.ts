import { Deck, Card, BLACKJACK, evaluate } from "./cards";
import { Player, Dealer, Hand } from "./player";
...
export class Game {
  ...
  private hit(hand: Hand) {
    ...
    if (hand.value > BLACKJACK)
      throw new Error(
        `Cannot hit a hand with a value greater than ${BLACKJACK}`
      );

    hand.cards.push(<Card>this.deck.cards.pop());
  ...
  private split() {
    ...
    if (this.player.hand.cards.length !== 2)
      throw new Error(
        `Cannot split a hand with ${this.player.hand.cards.length} cards`
      );
    ...
    this.player.hands.push(
      new Hand(this.player.hand.wager, [<Card>this.player.hand.cards.pop()])
    );
  ...
  private [GameState.Settle]() {
    ...
    this.state =
      this.deck.cards.length > this.minimumCards
        ? GameState.Deal
        : GameState.End;
